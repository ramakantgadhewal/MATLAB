...
for i = 1:ngp
   ...
   fe = fe + w(i)*N'*be;         % compute element nodal body force vector
                                 %      (integral the distributed forces)
end
...
for i=1:np        % loop over all point forces
   ...
   if xe(1)<=xpi && xpi<xe(nen)     % !!we can write it in this way for the
                                    % right endpoints of the bar belongs to B.C.
                                    % rather than the point force!!
